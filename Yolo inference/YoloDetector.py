...
class YoloDetector:
    ...
    def __init__(self, model_cfg, model_weights, classes):
        ...
        self.outputlayers = [self.layer_names[int(i) - 1] for i in self.net.getUnconnectedOutLayers()]
    ...
    def detect(self, img, conf=0.2, nms_thresh=0.2, non_max_suppression=True, class_conf=None):
        ...
        for key, index in indices.items():
            for i in index:
                select = i
                final_result[key].append(self.get_output_format(boxes[key][int(select)]))
